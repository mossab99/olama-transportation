    ...
    <div class="olama-tab-content">
        ...
        <?php if ($active_tab === 'reports'): ?>
            <div id="olama-school-reports" class="olama-card">
                ...
                <div class="olama-area-filters"><label>Report type<select id="school-report-type"><option value="school">School report</option><option value="walking">Walking students</option><option value="family">Family search</option><option value="unassigned">Transportation not assigned to trip</option></select></label></div>
                <section id="school-report-panel">
                <div class="olama-transportation-toolbar"><div><h2><?php echo esc_html($translate('School-level transportation report')); ?></h2><p class="description"><?php echo esc_html($translate('Select filters to view students enrolled in transportation. For students without transportation, use the Walking students report.')); ?></p></div></div>
                <div class="olama-area-filters"><label><?php echo esc_html($translate('Academic Year')); ?><select id="school-report-year"><?php foreach ($years as $year): ?><option value="<?php echo intval($year->id); ?>" <?php selected($selected_year_id,$year->id); ?>><?php echo esc_html($year->year_name); ?></option><?php endforeach; ?></select></label><label><?php echo esc_html($translate('Direction')); ?><select id="school-report-direction"><option value="morning"><?php echo esc_html($translate('Arrival / حضور')); ?></option><option value="afternoon"><?php echo esc_html($translate('Departure / عودة')); ?></option></select></label><label><?php echo esc_html($translate('Grade')); ?><select id="school-report-grade"><option value=""><?php echo esc_html($translate('All grades')); ?></option></select></label><label><?php echo esc_html($translate('Section')); ?><select id="school-report-section"><option value=""><?php echo esc_html($translate('All sections')); ?></option></select></label><label><?php echo esc_html($translate('Planning area')); ?><select id="school-report-area"><option value=""><?php echo esc_html($translate('All planning areas')); ?></option></select></label><label><?php echo esc_html($translate('Trip')); ?><select id="school-report-trip"><option value=""><?php echo esc_html($translate('All trips')); ?></option></select></label><label><?php echo esc_html($translate('Transportation')); ?><select id="school-report-transport"><option value="all"><?php echo esc_html($translate('With and without transportation')); ?></option><option value="with"><?php echo esc_html($translate('With transportation')); ?></option><option value="without"><?php echo esc_html($translate('Without transportation')); ?></option></select></label><button type="button" class="button" id="school-report-print"><?php echo esc_html($translate('Print report')); ?></button></div>
                <div id="school-report-feedback" class="olama-operation-result" aria-live="polite"></div><div id="school-report-results"></div>
                </section>
                <section id="family-report-panel" hidden><div id="family-report-controls"><label>Academic Year <select id="family-report-year"><?php foreach ($years as $year): ?><option value="<?php echo intval($year->id); ?>" <?php selected($selected_year_id,$year->id); ?>><?php echo esc_html($year->year_name); ?></option><?php endforeach; ?></select></label><label>Family ID, family name, student name, grade or section <input type="search" id="family-report-search"></label><button type="button" class="button" id="family-report-search-button">Search</button><button type="button" class="button" id="family-report-print">Print report</button></div><div id="family-report-results"></div></section>
                <section id="unassigned-report-panel" hidden><div class="olama-area-filters"><label>Academic Year <select id="unassigned-report-year"><?php foreach ($years as $year): ?><option value="<?php echo intval($year->id); ?>" <?php selected($selected_year_id,$year->id); ?>><?php echo esc_html($year->year_name); ?></option><?php endforeach; ?></select></label><button type="button" class="button" id="unassigned-report-print">Print report</button></div><div id="unassigned-report-results"></div></section>
            </div>
        <?php endif; ?>
            ...
            <div id="olama-area-planner" class="olama-card" data-year-id="<?php echo intval($selected_year_id); ?>">
                ...
                <div class="olama-area-planner-grid">
                    ...
                    <div class="olama-area-results">
                        <table class="wp-list-table widefat striped olama-area-allocation-table"><thead><tr><th><?php echo esc_html($translate('Planning Area')); ?></th><th><?php echo esc_html($translate('Families')); ?></th><th><?php echo esc_html($translate('Transportation Students')); ?></th><th><?php echo esc_html($translate('Transport KG + G1')); ?></th><th><?php echo esc_html($translate('Walking Students')); ?></th><th><?php echo esc_html($translate('Map Actions')); ?></th></tr></thead><tbody id="area-mapping-body"><tr><td colspan="6"><?php echo esc_html($translate('Loading...')); ?></td></tr></tbody></table>
        ...
        <?php if ($active_tab === 'settings'): ?>
            <div class="olama-card">
                ...
                <form id="transport-settings-form" class="olama-settings-form">
                    ...
                    <div data-optimizer-panel="ors"><h4>OpenRouteService</h4><label>API Key<input type="password" name="ors_api_key" value="" autocomplete="new-password" placeholder="<?php echo !empty($settings['ors_api_key']) || (defined('OLAMA_TRANSPORT_ORS_API_KEY') && OLAMA_TRANSPORT_ORS_API_KEY) ? esc_attr__('Configured', 'olama-transportation') : ''; ?>" /></label><label><?php echo esc_html($translate('Driving Profile')); ?><select name="ors_profile"><option value="driving-car" <?php selected($settings['ors_profile'] ?? 'driving-car', 'driving-car'); ?>>Driving Car</option><option value="driving-hgv" <?php selected($settings['ors_profile'] ?? '', 'driving-hgv'); ?>>Driving HGV</option><option value="cycling-regular" <?php selected($settings['ors_profile'] ?? '', 'cycling-regular'); ?>>Cycling</option><option value="foot-walking" <?php selected($settings['ors_profile'] ?? '', 'foot-walking'); ?>>Walking</option></select></label><p class="description">API Configuration Status: <?php echo (!empty($settings['ors_api_key']) || (defined('OLAMA_TRANSPORT_ORS_API_KEY') && OLAMA_TRANSPORT_ORS_API_KEY)) ? esc_html($translate('Configured')) : esc_html($translate('Missing')); ?></p><h4><?php echo esc_html($translate('Academy / Depot')); ?></h4><label>Latitude<input type="number" step="any" name="school_location[latitude]" value="<?php echo esc_attr($settings['school_location']['latitude'] ?? ''); ?>" /></label><label>Longitude<input type="number" step="any" name="school_location[longitude]" value="<?php echo esc_attr($settings['school_location']['longitude'] ?? ''); ?>" /></label><button type="button" class="button" id="test-ors-configuration">Test ORS Configuration</button></div>
                    <div data-optimizer-panel="google"><label>Google Project ID<input name="google_project_id" value="<?php echo esc_attr($settings['google_project_id'] ?? ''); ?>" /></label></div>
