...
class Olama_Transportation_ORS_Client
{
    ...
    private function post($path, array $payload)
    {
        ...
        $response = wp_safe_remote_post(self::BASE_URL . $path, array(
            'timeout' => 90,
            'redirection' => 0,
            'headers' => array('Authorization' => $this->get_api_key(), 'Content-Type' => 'application/json', 'Accept' => 'application/json'),
        ...
        if ($code < 200 || $code >= 300 || !is_array($body)) return new WP_Error('ors_request_failed', __('OpenRouteService returned an invalid response.', 'olama-transportation'));
        return $body;
    }
