(function () {
    ...
    function renderAreas() {
        ...
        areas.forEach(function (area) { var row = body.insertRow(), name = row.insertCell(), label = document.createElement('span'), dot = document.createElement('span'), actions = row.insertCell(); label.className = 'olama-area-name'; dot.className = 'olama-area-color-dot'; dot.style.setProperty('--area-color', color(area)); label.append(dot, node(area.name)); name.appendChild(label); row.insertCell().appendChild(node(area.family_count)); row.insertCell().appendChild(node(area.transportation_student_count || 0)); row.insertCell().appendChild(node(area.transport_kg_g1_count || 0)); row.insertCell().appendChild(node(area.non_transportation_student_count || 0)); [['all',olamaPlanner.i18n.mapAll||'Map all students'],['transportation',olamaPlanner.i18n.transportationMap||'Transportation map'],['walking',olamaPlanner.i18n.walkingMap||'Walking map']].forEach(function (item) { var button=document.createElement('button'); button.type='button'; button.className='button button-small'; button.appendChild(node(item[1])); button.addEventListener('click',function(event){event.stopPropagation();focusArea(area.id,item[0]);}); actions.appendChild(button); }); row.addEventListener('click', function (event) { if (!event.target.closest('button')) focusArea(area.id, 'all'); }); });
    }
