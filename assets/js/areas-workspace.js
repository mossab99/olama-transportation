(function ($) {
    ...
    function qrSvg(value){if(!value||typeof qrcode==='undefined')return '<span>No location</span>';var qr=qrcode(0,'M');qr.addData(value);qr.make();return qr.createSvgTag(2,0);}
    function badgeFace(student,trip,back){
        var siblings=(student.siblings||[]).map(function(s){return esc(s.student_name)+' ('+esc([s.grade_name,s.section_name].filter(Boolean).join(' / '))+')';}).join('، ')||'—';
        if(!back)return '<section class="badge front"><div class="lanyard"></div><header><b>OLAMA</b><small>Student Transportation</small></header><h1>'+esc(student.student_name)+'</h1><div class="hero"><div><label>Class / Grade</label><strong>'+esc([student.grade_name,student.section_name].filter(Boolean).join(' / ')||'—')+'</strong><label>Bus / Trip</label><strong>'+esc(trip.bus_number||'—')+' / '+esc(trip.bus_trip_number||'—')+'</strong><label>Area</label><strong>'+esc(student.area_name||'—')+'</strong></div><div class="qr">'+qrSvg(student.maps_url)+'</div></div><footer>#'+esc(student.oracle_student_id||student.student_uid)+'</footer></section>';
        return '<section class="badge back"><div class="lanyard"></div><h2>Transportation Details</h2><dl><dt>Driver</dt><dd>'+esc(trip.driver_name||'—')+' '+esc(trip.driver_phone||'')+'</dd><dt>Companion</dt><dd>'+esc(trip.companion_name||'—')+' '+esc(trip.companion_phone||'')+'</dd><dt>Father</dt><dd>'+esc(student.father_name||'—')+' '+esc(student.father_mobile||'')+'</dd><dt>Mother</dt><dd>'+esc(student.mother_name||'—')+' '+esc(student.mother_mobile||'')+'</dd><dt>Siblings</dt><dd>'+siblings+'</dd><dt>Address</dt><dd>'+esc(student.full_address||'—')+'</dd></dl><small>If found, please return this badge to the school.</small></section>';
    }
    function printBadges(tripId){
        var win=window.open('','_blank');if(!win){feedback('Allow pop-ups to preview the badges.',true);return;}win.document.write('<p style="font-family:Arial;padding:24px">Preparing badges…</p>');
        feedback('Preparing badges…');api('areas-workspace/shared-trips/'+tripId+'/badges').then(function(payload){
            ...
            var cards=payload.students.map(function(student){return '<div class="student-badge"><div class="screen-name">'+esc(student.student_name)+' <button onclick="printOne(this)">Print this badge</button></div>'+badgeFace(student,payload.trip,false)+badgeFace(student,payload.trip,true)+'</div>';}).join('');
            var style='@page{size:54mm 85.6mm;margin:0}*{box-sizing:border-box}body{margin:0;background:#eef1f5;font-family:Arial,Tahoma,sans-serif}.toolbar{position:sticky;top:0;z-index:2;padding:12px;text-align:center;background:#fff}.student-badge{display:flex;gap:12px;justify-content:center;flex-wrap:wrap;margin:18px}.screen-name{flex-basis:100%;text-align:center;font-weight:700}.badge{position:relative;width:54mm;height:85.6mm;overflow:hidden;padding:9mm 4mm 4mm;background:#fff;border:1px solid #bcc5d3;border-radius:3mm;color:#172033}.lanyard{position:absolute;top:2mm;left:50%;width:13mm;height:3mm;transform:translateX(-50%);border:1px solid #b5bdc9;border-radius:3mm}.badge header{text-align:center;padding:2mm 0;border-bottom:1px solid #d8dfeb}.badge header b,.badge header small{display:block}.badge h1{margin:4mm 0 3mm;text-align:center;font-size:5mm;line-height:1.15}.hero{display:grid;grid-template-columns:1fr 22mm;gap:2mm}.badge label{display:block;margin-top:2mm;color:#667085;font-size:2.3mm}.badge strong{display:block;font-size:3.1mm}.qr svg{width:21mm;height:21mm}.badge footer{position:absolute;bottom:3mm;left:4mm;right:4mm;text-align:center;font-size:2.5mm}.back h2{margin:2mm 0 3mm;text-align:center;font-size:4mm}.back dl{margin:0;font-size:2.65mm}.back dt{margin-top:1.4mm;color:#667085;font-weight:700}.back dd{margin:0;line-height:1.25;overflow-wrap:anywhere}.back>small{position:absolute;bottom:3mm;left:4mm;right:4mm;text-align:center;font-size:2.2mm}@media print{body{background:#fff}.toolbar,.screen-name{display:none}.student-badge{display:block;margin:0}.badge{page-break-after:always;border:0;border-radius:0}.student-badge.is-hidden{display:none}}';
            win.document.write('<!doctype html><html><head><meta charset="utf-8"><title>Student bus badges</title><style>'+style+'</style></head><body><div class="toolbar"><button onclick="window.print()">Print all badges</button> &nbsp; CR80 · 54 × 85.6 mm · duplex</div>'+cards+'<script>function printOne(b){document.querySelectorAll(".student-badge").forEach(function(x){x.classList.toggle("is-hidden",x!==b.closest(".student-badge"));});window.print();document.querySelectorAll(".student-badge").forEach(function(x){x.classList.remove("is-hidden");});}<\/script></body></html>');win.document.close();feedback('');
    ...
    function renderQueueMap(){
        ...
        valid.forEach(function(node){
            ...
            var icon=L.divIcon({className:'olama-queue-map-icon-wrap',html:'<span class="olama-queue-map-marker '+(isSchool?'is-school':'is-family')+'">'+(isSchool?'S':index)+'</span>',iconSize:[38,38],iconAnchor:[19,19],popupAnchor:[0,-20]});
    ...
    function renderQueue(){
        ...
        var nodes=queue.map(function(node,index){var cls=node.node_type==='school'?'is-school':(validCoordinates(node)?'is-family':'is-missing');var label=node.node_type==='school'?'School':'View details for family '+(node.oracle_family_id||node.family_name);return '<li class="'+cls+'" data-node-index="'+index+'" tabindex="0" role="button" aria-label="'+esc(label)+'"><span>'+esc(node.node_type==='school'?'School':(node.oracle_family_id||node.family_name))+'</span><small>'+(node.node_type==='school'?(wizard.trip.direction==='morning'?'Final stop':'Starting stop'):(node.student_count+' students'+(!validCoordinates(node)?' · Location missing':'')))+'</small></li>';}).join('');
    ...
    $(function(){
        ...
        $(document).on('change','.areas-type,.areas-color',function(){var row=$(this).closest('tr');api('areas-workspace/'+row.data('area'),{method:'PUT',body:JSON.stringify({area_type:row.find('.areas-type').prop('checked')?'main':'secondary',color:row.find('.areas-color').val()})}).then(function(){return load('Area settings saved.');}).catch(function(error){feedback(error.message,true);});});
        ...
        $(document).on('change','.olama-trip-student',function(){rememberStudentSelection();updateStudentSelectionCount();});
