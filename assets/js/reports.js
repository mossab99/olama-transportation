(function () {
    ...
    function familyReport() {
        ...
        fetch(olamaReports.restUrl + 'reports/families?' + query.toString(), {headers: {'X-WP-Nonce': olamaReports.restNonce}}).then(function (response) { return response.json(); }).then(function (data) {
            var html = (data.items || []).map(function (family) { var rows = (family.transport_rows || []).map(function (row, index) { return '<tr><td>' + (index + 1) + '</td><td>' + esc(row.student_name) + '</td><td>' + esc(row.grade_name || '—') + '</td><td>' + esc(row.section_name || '—') + '</td><td>' + esc(row.direction || '—') + '</td><td>' + esc(row.trip_name || '—') + '</td><td>' + esc(row.driver_name || '—') + '</td><td>' + esc(row.bus_number || '—') + '</td></tr>'; }).join(''); var map = family.maps_url ? '<a target="_blank" rel="noopener" href="' + esc(family.maps_url) + '">Map location</a>' : '—'; return '<section class="family-report-card"><h3>' + esc(family.family_name) + ' (#' + esc(family.oracle_family_id) + ')</h3><p>Father: ' + esc(family.father_mobile || '—') + ' · Mother: ' + esc(family.mother_mobile || '—') + ' · Address: ' + esc(family.oracle_address || '—') + ' · ' + map + '</p><table><thead><tr><th>#</th><th>Student</th><th>Grade</th><th>Section</th><th>Direction</th><th>Trip</th><th>Driver</th><th>Bus #</th></tr></thead><tbody>' + (rows || '<tr><td colspan="8">No trip assignment</td></tr>') + '</tbody></table></section>'; }).join(''); $('#family-report-results').innerHTML = html || '<p>No matching families found.</p>'; $('#school-report-feedback').textContent = ''; }).catch(function (error) { $('#school-report-feedback').textContent = error.message; });
    }
    function unassignedReport() {
        $('#school-report-feedback').textContent = 'Loading unassigned transportation students…'; fetch(olamaReports.restUrl + 'reports/unassigned?academic_year_id=' + encodeURIComponent($('#unassigned-report-year').value), {headers: {'X-WP-Nonce': olamaReports.restNonce}}).then(function (response) { return response.json(); }).then(function (data) { var body = (data.rows || []).map(function (row, index) { var map = row.maps_url ? '<a target="_blank" rel="noopener" href="' + esc(row.maps_url) + '">Map</a>' : '—'; return '<tr><td>' + (index + 1) + '</td><td>' + esc(row.student_name) + '</td><td>' + esc(row.grade_name || '—') + '</td><td>' + esc(row.section_name || '—') + '</td><td>' + esc(row.oracle_family_id || '—') + '</td><td dir="ltr">' + esc(row.father_mobile || '—') + '</td><td dir="ltr">' + esc(row.mother_mobile || '—') + '</td><td>' + esc(row.oracle_address || '—') + '<br>' + map + '</td></tr>'; }).join(''); $('#unassigned-report-results').innerHTML = '<strong>' + (data.rows || []).length + ' students</strong><table class="wp-list-table widefat striped"><thead><tr><th>#</th><th>Student</th><th>Grade</th><th>Section</th><th>Family ID</th><th>Father mobile</th><th>Mother mobile</th><th>Address / map</th></tr></thead><tbody>' + (body || '<tr><td colspan="8">No unassigned students found.</td></tr>') + '</tbody></table>'; $('#school-report-feedback').textContent = ''; }).catch(function (error) { $('#school-report-feedback').textContent = error.message; });
    }
